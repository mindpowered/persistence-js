...
class persistence_Persistence {
	constructor(bus) {
		...
		this.maglev.register("Persistence.AddGetter",function(args) {
			...
			let operationName = args[1];
			let strategyMethod = args[2];
			let queryMapper = args[3];
			let resultMapper = args[4];
	...
	addMutator(recordType,operationName,strategyMethod,updateMapper) {
		...
		let mutator = new persistence__$Persistence_Mutator(recordType,strategyMethod,updateMapper);
		this.mutators.h[recordType + "." + operationName] = mutator;
	}
	addGetter(recordType,operationName,strategyMethod,queryMapper,resultMapper) {
		...
		let getter = new persistence__$Persistence_Getter(recordType,strategyMethod,queryMapper,resultMapper);
...
class persistence__$Persistence_Mutator {
	constructor(recordType,strategyMethod,updateMapper) {
...
class persistence__$Persistence_Getter {
	constructor(recordType,strategyMethod,queryMapper,resultMapper) {
		if(typeof(strategyMethod) != "string" && !Reflect.isFunction(strategyMethod)) {
			throw haxe_Exception.thrown("strategyMethod must be a string or function");
		}
