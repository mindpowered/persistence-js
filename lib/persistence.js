...
class persistence_Persistence {
	constructor(bus) {
		...
		this.maglev.register("Persistence.AddMutator",function(args) {
			let recordType = args[0];
			let operationName = args[1];
			let strategyMethod = args[2];
		...
		this.maglev.register("Persistence.AddGetter",function(args) {
			...
			if(args.length >= 6) {
				useQueryValuesAsParams = args[5];
			}
			_gthis.addGetter(recordType,operationName,strategyMethod,queryMapper,resultMapper,useQueryValuesAsParams);
			return true;
		});
		this.maglev.register("Persistence.Mutate",function(args) {
			let recordType = args[0];
			let operationName = args[1];
			let recordData = args[2];
			_gthis.mutate(recordType,operationName,recordData);
			return true;
		});
		this.maglev.register("Persistence.Get",function(args) {
			let recordType = args[0];
			let operationName = args[1];
			let queryValues = args[2];
