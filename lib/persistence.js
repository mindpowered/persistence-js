...
$hx_exports["persistence"] = $hx_exports["persistence"] || {};
...
class persistence_MagLevConverter {
	static convertToHaxe(x) {
		if(x.getType() == maglev_MagLevNull.getStaticType()) {
			...
			let i = 0;
			while(i < y.size()) {
				arr.push(persistence_MagLevConverter.convertToHaxe(y.get(i)));
				++i;
			}
			return arr;
			...
			while(i < keys.size()) {
				...
				let value = persistence_MagLevConverter.convertToHaxe(y.get(key));
				map.h[key] = value;
				++i;
			}
			return map;
	...
	static convertToMagLev(x) {
		if(x == null) {
			...
			let f = function(args) {
				let y = x;
				let haxeArgs = persistence_MagLevConverter.convertToHaxe(args);
				let r = maglev_MagLevResult.createAsync();
				y(haxeArgs,function(haxeRet) {
			...
			while(key.hasNext()) {
				let key1 = key.next();
				obj.set(key1,persistence_MagLevConverter.convertToMagLev(map.h[key1]));
			}
			return obj;
...
$hxClasses["persistence.MagLevConverter"] = persistence_MagLevConverter;
persistence_MagLevConverter.__name__ = "persistence.MagLevConverter";
class persistence_Persistence {
	constructor(bus) {
		this.mutators = new haxe_ds_StringMap();
		this.getters = new haxe_ds_StringMap();
		let _gthis = this;
		this.maglev = bus;
		this.maglev.register("Persistence.AddMutator",maglev_MagLevFunction.fromFunction(function(args) {
			let recordType = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let operationName = (js_Boot.__cast(args.get(1) , maglev_MagLevString)).getString();
			let strategyMethod = _gthis.convertToHaxe(args.get(2));
			let updateMapper = null;
			if(args.size() > 3) {
				updateMapper = _gthis.convertToHaxe(args.get(3));
			}
			let useRecordDataAsParams = null;
			if(args.size() > 4) {
		...
		this.maglev.register("Persistence.AddGetter",maglev_MagLevFunction.fromFunction(function(args) {
			let recordType = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let operationName = (js_Boot.__cast(args.get(1) , maglev_MagLevString)).getString();
			let strategyMethod = _gthis.convertToHaxe(args.get(2));
			let queryMapper = null;
			if(args.size() >= 4) {
				queryMapper = _gthis.convertToHaxe(args.get(3));
			}
			let resultMapper = null;
			if(args.size() >= 5) {
				resultMapper = _gthis.convertToHaxe(args.get(4));
			}
			let useQueryValuesAsParams = null;
			if(args.size() >= 6) {
				useQueryValuesAsParams = (js_Boot.__cast(args.get(5) , maglev_MagLevBoolean)).getBool();
			}
			_gthis.addGetter(recordType,operationName,strategyMethod,queryMapper,resultMapper,useQueryValuesAsParams);
			return maglev_MagLevResult.fromResult(maglev_MagLevBoolean.fromBool(true));
		}));
		this.maglev.register("Persistence.Mutate",maglev_MagLevFunction.fromFunction(function(args) {
			let recordType = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let operationName = (js_Boot.__cast(args.get(1) , maglev_MagLevString)).getString();
			let recordData = _gthis.convertToHaxe(js_Boot.__cast(args.get(2) , maglev_MagLevArray));
			let maglevResult = maglev_MagLevResult.createAsync();
			_gthis.mutate(recordType,operationName,recordData,function(haxeResult) {
				maglevResult.setResult(_gthis.convertToMagLev(haxeResult));
			});
			return maglevResult;
		}));
		this.maglev.register("Persistence.Get",maglev_MagLevFunction.fromFunction(function(args) {
			let recordType = (js_Boot.__cast(args.get(0) , maglev_MagLevString)).getString();
			let operationName = (js_Boot.__cast(args.get(1) , maglev_MagLevString)).getString();
			let queryValues = _gthis.convertToHaxe(js_Boot.__cast(args.get(2) , maglev_MagLevArray));
			let maglevResult = maglev_MagLevResult.createAsync();
			_gthis.get(recordType,operationName,queryValues,function(haxeResult) {
				maglevResult.setResult(_gthis.convertToMagLev(haxeResult));
			});
			return maglevResult;
		}));
	...
	addMutator(recordType,operationName,strategyMethod,updateMapper,useRecordDataAsParams) {
		...
		let mutator = new persistence__$Persistence_Mutator(recordType,strategyMethod,updateMapper,useRecordDataAsParams);
		let this1 = this.mutators;
		let k = this.calcKey(recordType,operationName);
		this1.h[k] = mutator;
	}
	addGetter(recordType,operationName,strategyMethod,queryMapper,resultMapper,useQueryValuesAsParams) {
		...
		let getter = new persistence__$Persistence_Getter(recordType,strategyMethod,queryMapper,resultMapper,useQueryValuesAsParams);
		let this1 = this.getters;
		let k = this.calcKey(recordType,operationName);
		this1.h[k] = getter;
	}
	mutate(recordType,operationName,recordData,callback) {
		let _gthis = this;
		let this1 = this.mutators;
		let key = this.calcKey(recordType,operationName);
		if(Object.prototype.hasOwnProperty.call(this1.h,key)) {
			let this1 = this.mutators;
			let key = this.calcKey(recordType,operationName);
			let mutator = this1.h[key];
			this.mapUpdate(mutator,recordData,function(mappedRecordData) {
				_gthis.update(mutator,mappedRecordData,function(updateResults) {
					callback(updateResults);
				});
			});
		} else {
			throw haxe_Exception.thrown("the specified mutator has not been added");
		}
	}
	get(recordType,operationName,queryValues,callback) {
		let _gthis = this;
		let this1 = this.getters;
		let key = this.calcKey(recordType,operationName);
		if(Object.prototype.hasOwnProperty.call(this1.h,key)) {
			let this1 = this.getters;
			let key = this.calcKey(recordType,operationName);
			let getter = this1.h[key];
			this.mapQuery(getter,queryValues,function(mappedQueryValues) {
				_gthis.query(getter,mappedQueryValues,function(queryResults) {
					_gthis.mapResults(getter,queryResults,function(mappedQueryValues) {
						callback(mappedQueryValues);
					});
				});
			});
		} else {
			throw haxe_Exception.thrown("the specified getter has not been added");
		}
	}
	calcKey(recordType,operationName) {
		return recordType + "." + operationName;
	}
	convertToHaxe(x) {
	...
	mapUpdate(mutator,recordData,callback) {
		let _gthis = this;
		if(Reflect.isFunction(mutator.updateMapper)) {
			let updateMapper = mutator.updateMapper;
			updateMapper(recordData,function(params) {
				callback(params);
			});
		} else if(typeof(mutator.updateMapper) == "string") {
			let updateMapper = mutator.updateMapper;
			let ret = this.maglev.call(updateMapper,js_Boot.__cast(this.convertToMagLev([recordData]) , maglev_MagLevArray));
			ret.onError(function(error) {
			...
			throw haxe_Exception.thrown("updateMapper must be a string or function");
		}
	}
	update(mutator,recordData,callback) {
		let _gthis = this;
		if(Reflect.isFunction(mutator.strategyMethod)) {
			let strategyMethod = mutator.strategyMethod;
			strategyMethod(recordData,function(raw_result) {
				callback(raw_result);
			});
		} else if(typeof(mutator.strategyMethod) == "string") {
			let strategyMethod = mutator.strategyMethod;
			let raw_maglev_result;
			if(mutator.useRecordDataAsParams) {
				raw_maglev_result = this.maglev.call(strategyMethod,js_Boot.__cast(this.convertToMagLev(recordData) , maglev_MagLevArray));
			} else {
				raw_maglev_result = this.maglev.call(strategyMethod,js_Boot.__cast(this.convertToMagLev([recordData]) , maglev_MagLevArray));
			}
			...
			raw_maglev_result.onResult(function(result) {
				callback(_gthis.convertToHaxe(result));
				return maglev_MagLevResult.fromResult(maglev_MagLevNull.create());
			});
		} else {
			throw haxe_Exception.thrown("strategyMethod must be a string or function");
		}
	}
	mapQuery(getter,queryValues,callback) {
		let _gthis = this;
		if(Reflect.isFunction(getter.queryMapper)) {
			let queryMapper = getter.queryMapper;
			queryMapper(queryValues,function(mappedQueryValues) {
				callback(mappedQueryValues);
			});
		} else if(typeof(getter.queryMapper) == "string") {
			let queryMapper = getter.queryMapper;
			let ret = this.maglev.call(queryMapper,js_Boot.__cast(this.convertToMagLev([queryValues]) , maglev_MagLevArray));
			ret.onError(function(error) {
				throw haxe_Exception.thrown(error.getMessage());
			});
			ret.onResult(function(result) {
			...
			throw haxe_Exception.thrown("queryMapper must be a string or function");
		}
	}
	query(getter,queryValues,callback) {
		let raw_result = null;
		let _gthis = this;
		if(Reflect.isFunction(getter.strategyMethod)) {
			let strategyMethod = getter.strategyMethod;
			strategyMethod(queryValues,function(raw_result) {
				callback(raw_result);
			});
		} else if(typeof(getter.strategyMethod) == "string") {
			let strategyMethod = getter.strategyMethod;
			let raw_maglev_result;
			if(getter.useQueryValuesAsParams) {
				raw_maglev_result = this.maglev.call(getter.strategyMethod,js_Boot.__cast(this.convertToMagLev(queryValues) , maglev_MagLevArray));
			} else {
				raw_maglev_result = this.maglev.call(getter.strategyMethod,js_Boot.__cast(this.convertToMagLev([queryValues]) , maglev_MagLevArray));
			}
			raw_maglev_result.onError(function(error) {
	...
	mapResults(getter,queryResults,callback) {
		let _gthis = this;
		if(Reflect.isFunction(getter.resultMapper)) {
			let resultMapper = getter.resultMapper;
			resultMapper(queryResults,function(mappedResults) {
				callback(mappedResults);
			});
		} else if(typeof(getter.resultMapper) == "string") {
			let resultMapper = getter.resultMapper;
			let maglev_result = this.maglev.call(resultMapper,js_Boot.__cast(this.convertToMagLev([queryResults]) , maglev_MagLevArray));
			maglev_result.onError(function(error) {
				throw haxe_Exception.thrown(error.getMessage());
			});
			maglev_result.onResult(function(result) {
				callback(_gthis.convertToHaxe(result));
				return maglev_MagLevResult.fromResult(maglev_MagLevNull.create());
			});
		} else {
			throw haxe_Exception.thrown("resultMapper must be a string or function");
		}
	}
}
$hxClasses["persistence.Persistence"] = $hx_exports["persistence"]["Persistence"] = persistence_Persistence;
persistence_Persistence.__name__ = "persistence.Persistence";
Object.assign(persistence_Persistence.prototype, {
	__class__: persistence_Persistence
	,mutators: null
	,getters: null
	,maglev: null
});
class persistence__$Persistence_Mutator {
	constructor(recordType,strategyMethod,updateMapper,useRecordDataAsParams) {
		if(typeof(strategyMethod) != "string" && !Reflect.isFunction(strategyMethod)) {
			throw haxe_Exception.thrown("strategyMethod must be a string or function");
		}
		if(typeof(updateMapper) != "string" && !Reflect.isFunction(updateMapper)) {
			throw haxe_Exception.thrown("updateMapper must be a string or function");
		}
...
$hxClasses["persistence._Persistence.Mutator"] = persistence__$Persistence_Mutator;
persistence__$Persistence_Mutator.__name__ = "persistence._Persistence.Mutator";
Object.assign(persistence__$Persistence_Mutator.prototype, {
	__class__: persistence__$Persistence_Mutator
	,recordType: null
	,strategyMethod: null
	,updateMapper: null
	,useRecordDataAsParams: null
});
class persistence__$Persistence_Getter {
	constructor(recordType,strategyMethod,queryMapper,resultMapper,useQueryValuesAsParams) {
		if(typeof(strategyMethod) != "string" && !Reflect.isFunction(strategyMethod)) {
			throw haxe_Exception.thrown("strategyMethod must be a string or function");
		}
		if(typeof(queryMapper) != "string" && !Reflect.isFunction(queryMapper)) {
			throw haxe_Exception.thrown("queryMapper must be a string or function");
		}
		if(typeof(resultMapper) != "string" && !Reflect.isFunction(resultMapper)) {
			throw haxe_Exception.thrown("resultMapper must be a string or function");
		}
		this.recordType = recordType;
		this.strategyMethod = strategyMethod;
...
$hxClasses["persistence._Persistence.Getter"] = persistence__$Persistence_Getter;
persistence__$Persistence_Getter.__name__ = "persistence._Persistence.Getter";
Object.assign(persistence__$Persistence_Getter.prototype, {
	__class__: persistence__$Persistence_Getter
	,recordType: null
	,strategyMethod: null
	,queryMapper: null
	,resultMapper: null
...
persistence_Persistence.__meta__ = { obj : { SuppressWarnings : [["checkstyle:FieldDocComment","checkstyle:LocalVariableName","checkstyle:MultipleStringLiterals","checkstyle:MagicNumber"]]}};
persistence__$Persistence_Mutator.__meta__ = { obj : { SuppressWarnings : ["checkstyle:FieldDocComment"]}};
persistence__$Persistence_Getter.__meta__ = { obj : { SuppressWarnings : ["checkstyle:FieldDocComment"]}};
})(typeof window != "undefined" ? window : typeof global != "undefined" ? global : typeof self != "undefined" ? self : this);
var persistence = $hx_exports["persistence"];
