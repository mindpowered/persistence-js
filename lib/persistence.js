...
class persistence_Persistence {
	constructor(bus) {
		...
		this.maglev.register("Persistence.Mutate",maglev_MagLevFunction.fromFunction(function(args) {
			...
			let ret = _gthis.convertToMagLev(_gthis.mutate(recordType,operationName,recordData));
		...
		this.maglev.register("Persistence.Get",maglev_MagLevFunction.fromFunction(function(args) {
			...
			let ret = _gthis.convertToMagLev(_gthis.get(recordType,operationName,queryValues));
			return maglev_MagLevResult.fromResult(ret);
		}));
	}
	...
	mutate(recordType,operationName,recordData) {
		...
		if(Object.prototype.hasOwnProperty.call(this1.h,key)) {
			...
			if(Reflect.isFunction(mutator.updateMapper)) {
				...
				let updateMapper = mutator.updateMapper;
				params = this.maglev.call(updateMapper,js_Boot.__cast(this.convertToMagLev([recordData]) , maglev_MagLevArray));
			} else {
			...
			if(Reflect.isFunction(mutator.strategyMethod)) {
				...
				if(mutator.useRecordDataAsParams) {
					raw_maglev_result = this.maglev.call(strategyMethod,js_Boot.__cast(this.convertToMagLev(params) , maglev_MagLevArray));
				} else {
					raw_maglev_result = this.maglev.call(strategyMethod,js_Boot.__cast(this.convertToMagLev([params]) , maglev_MagLevArray));
				}
				if(raw_maglev_result.isError()) {
					throw haxe_Exception.thrown(raw_maglev_result.getError().getMessage());
				}
	...
	get(recordType,operationName,queryValues) {
		...
		if(Object.prototype.hasOwnProperty.call(this1.h,key)) {
			...
			if(Reflect.isFunction(getter.queryMapper)) {
				...
				let queryMapper = getter.queryMapper;
				query = this.maglev.call(queryMapper,js_Boot.__cast(this.convertToMagLev([queryValues]) , maglev_MagLevArray));
			} else {
			...
			if(Reflect.isFunction(getter.strategyMethod)) {
				...
				if(getter.useQueryValuesAsParams) {
					raw_maglev_result = this.maglev.call(getter.strategyMethod,js_Boot.__cast(this.convertToMagLev(query) , maglev_MagLevArray));
				} else {
					raw_maglev_result = this.maglev.call(getter.strategyMethod,js_Boot.__cast(this.convertToMagLev([query]) , maglev_MagLevArray));
				}
				if(raw_maglev_result.isError()) {
			...
			if(Reflect.isFunction(getter.resultMapper)) {
				...
				let resultMapper = getter.resultMapper;
				let maglev_result = this.maglev.call(resultMapper,js_Boot.__cast(this.convertToMagLev([raw_result]) , maglev_MagLevArray));
				if(maglev_result.isError()) {
					throw haxe_Exception.thrown(maglev_result.getError().getMessage());
				}
